import { getCurrentUserId } from '@/lib/auth/session';
import { DataAccessService, TransactionFilters } from '@/lib/services/data-access.service';
import { NextRequest, NextResponse } from 'next/server';

/**
 * GET /api/sharing/transactions
 *
 * @deprecated This endpoint is deprecated. Use Appwrite Realtime subscriptions instead.
 *
 * All transaction data (own + shared) should be fetched via the `useTransactionsWithSharing` hook
 * which uses Appwrite Realtime for automatic updates.
 *
 * @see hooks/useTransactionsWithSharing.ts
 * @see docs/REALTIME_USAGE_GUIDE.md
 */
export async function GET(request: NextRequest) {
  return NextResponse.json(
    {
      success: false,
      message:
        'This endpoint is deprecated. Use Appwrite Realtime subscriptions via useTransactionsWithSharing hook instead.',
      migration: {
        hook: 'useTransactionsWithSharing',
        docs: '/docs/REALTIME_USAGE_GUIDE.md',
      },
    },
    { status: 410 },
  );
}
