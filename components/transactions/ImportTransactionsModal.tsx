...
import { ERROR_MESSAGES, ImportErrorCode, getErrorMessage } from '@/lib/types/import.types';
        ...
        <div className="p-6">
          ...
          {state.step === 'upload' && (
            <div className="space-y-5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <ol className="text-sm text-text-secondary space-y-1 list-decimal list-inside">
                      ...
                      <li>Faça upload do arquivo de extrato (OFX, CSV ou PDF)</li>
                    ...
                    <details className="mt-3">
                      ...
                      <div className="mt-2 pl-4 space-y-2 text-xs text-text-secondary">
                        ...
                        {process.env.NEXT_PUBLIC_ENABLE_PDF_IMPORT === 'true' && (
                          <div>
                            <strong className="text-text-primary">PDF (.pdf) - Beta</strong>
                            <p>Processado com IA. Pode requerer revisão manual.</p>
                          </div>
                        )}
                      </div>
                    </details>
              ...
              {/* PDF Beta Warning */}
              {state.file && state.file.name.toLowerCase().endsWith('.pdf') && (
                <div
                  className="bg-yellow-bg border border-yellow-border rounded-lg p-4"
                  role="alert"
                  aria-live="polite"
                >
                  <div className="flex items-start gap-3">
                    <svg
                      className="w-5 h-5 text-yellow-text mt-0.5 flex-shrink-0"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      aria-hidden="true"
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-yellow-text mb-1">
                        Importação de PDF em Beta
                      </p>
                      <p className="text-sm text-text-secondary">
                        A importação de arquivos PDF usa inteligência artificial e está em fase beta. 
                        Os resultados podem precisar de revisão manual. Para maior precisão, recomendamos 
                        usar arquivos OFX ou CSV quando disponíveis.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  {!state.file ? (
                    <>
                      ...
                      <p className="text-xs text-text-tertiary">
                        Formatos suportados: OFX, CSV{process.env.NEXT_PUBLIC_ENABLE_PDF_IMPORT === 'true' ? ', PDF (Beta)' : ''} (máx. 10MB)
                      </p>
                      {process.env.NEXT_PUBLIC_ENABLE_PDF_IMPORT !== 'true' && (
                        <p className="text-xs text-yellow-text mt-2">
                          📄 Importação de PDF em breve
                        </p>
                      )}
                    </>
                  ) : (
